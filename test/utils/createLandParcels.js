...
const inputPath = path.join(__dirname, '../base-land-data/parcels/parcels.csv')
const outputDir = path.join(__dirname, '../land-data/parcels')
const outputCsvPath = path.join(outputDir, 'parcels_test.csv')
const outputZipPath = path.join(outputDir, 'parcels_test.zip')
...
export async function createLandParcels(
  ...
  // Update first two rows to make them unique
  if (rows.length > 0) {
  ...
  await createZipFromCsv(outputCsvPath, outputZipPath, 'parcels_test.csv')
