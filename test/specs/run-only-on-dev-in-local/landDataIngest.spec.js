...
- Creates a new land parcel csv and zip files with one new unique fake land parcel and one existing land parcel
- Creates a new land cover csv and zip files with land covers for above fake land parcel and land covers for one existing land parcel
- Upload these zip and csv files to s3 bucket
...
createLandParcels(SHEET_ID1, PARCEL_ID1, SHEET_ID2, PARCEL_ID2)
createLandCovers(SHEET_ID1, PARCEL_ID1, SHEET_ID2, PARCEL_ID2)

const fileVariants = [
  [
    'zip',
    'test/land-data/parcels/parcels_test.zip',
    'test/land-data/covers/covers_test.zip'
  ],
  [
    'csv',
    'test/land-data/parcels/parcels_test.csv',
    'test/land-data/covers/covers_test.csv'
  ]
]

describe.each(fileVariants)(
  'Land data ingest through (%s) files',
  (_ext, parcelsFile, coversFile) => {
    it('should validate land parcels and land covers ingested', async () => {
      ...
      const parcel1 = res.body.parcels.find(
        (parcel) =>
          parcel.sheetId === SHEET_ID1 && parcel.parcelId === PARCEL_ID1
      )
      ...
      const parcel2 = res.body.parcels.find(
        (parcel) =>
          parcel.sheetId === SHEET_ID2 && parcel.parcelId === PARCEL_ID2
      )
      ...
      console.log('validating parcel1:', [parcel1])
      ...
      console.log('validating parcel2:', [parcel2])
